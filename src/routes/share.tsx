...
import { useAddSharedRecipe } from "@/lib/useUserStats";
...
function SharePage() {
  ...
  const { user } = useAuth();
  const { addRecipe } = useAddSharedRecipe();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Add to Supabase shared_recipes table if user is authenticated
      if (user?.id) {
        try {
          ...
          // Still continue - local storage worked
        }
      }
